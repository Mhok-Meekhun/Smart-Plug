...
function scheduleNotFound() {
  return new NotFoundException({
    ...
    messageKey: "errors.scheduleNotFound",
  });
}
...
function assertTimezone(timezone: string) {
  try {
    ...
    throw new BadRequestException({
      ...
      messageKey: "errors.invalidTimezone",
    });
  }
}
...
export class SchedulesService {
  ...
  async listExecutions(userId: string, scheduleId: string) {
    ...
    return this.prisma.scheduleExecution.findMany({
      ...
      include: {
        command: {
          select: { status: true, acknowledgedAt: true, failureCode: true },
        },
      },
  ...
  async create(userId: string, input: CreateSchedule) {
    const device = await this.prisma.device.findFirst({
      where: {
        id: input.deviceId,
        home: { members: { some: { userId, role: "OWNER" } } },
      },
      select: { id: true },
    });
    if (!device) {
    ...
    if (executeAt && executeAt <= now) {
      throw new BadRequestException({
        ...
        messageKey: "errors.scheduleInPast",
      });
    }
    const weekdays =
      input.kind === "REPEATING" ? [...new Set(input.weekdays)].sort() : [];
    const nextRunAt =
      input.kind === "ONCE"
        ? executeAt!
        : nextRepeatingRun(
            { localTime: input.localTime, weekdays, timezone: input.timezone },
            now,
          );
    ...
    return this.prisma.deviceSchedule.create({
      data: {
        ...
        executeAt,
        localTime:
          input.kind === "REPEATING"
            ? new Date(`1970-01-01T${input.localTime}:00.000Z`)
            : null,
        weekdays,
        nextRunAt,
      },
  ...
  async setEnabled(userId: string, scheduleId: string, enabled: boolean) {
    const schedule = await this.prisma.deviceSchedule.findFirst({
      where: {
        id: scheduleId,
        device: { home: { members: { some: { userId, role: "OWNER" } } } },
      },
    });
    if (!schedule) throw scheduleNotFound();
